...
def load_or_create_prompt(prompt_path: str) -> str:
    ...
    if not os.path.exists(prompt_path):
        prompt = (
            "You are an expert in ENS (Ethereum Name Service) name-based identity analysis.\n"
            "Your task is to cluster ENS names into plausible entities based ONLY on string similarity.\n\n"
            "Definition:\n"
            "- SAME entity means it is plausible/likely the same on-chain owner/controller controls those ENS names\n"
            "  (e.g., naming conventions, consistent prefixes/suffixes, brand/persona patterns, systematic numbering, wallet/project naming).\n"
            "- You only see ENS names as strings (no on-chain data). Use reasonable inference.\n\n"
            "Policy:\n"
            ...
            "- ENS names of the SAME entity are separated by commas\n"
            "- DIFFERENT entities are separated by newline\n"
            "- Output ONLY the entities\n\n\n"
            "****MOST IMPORTANT:\n****"
                "During the process, you need to evaluate the entities on a line. There may be a wrong case like:\n"
                "    \"moonbird,bluemoonbird,bluemoon,bluesun\"\n"
                "    It would be: \"moonbird,bluemoonbird\"\n\"bluemoon,bluesun\"\n"
                "In this case, consider the following logic:\n"
                "- 'moonbird' and 'bluemoonbird' are likely the SAME entity due to shared brand/entity signals and common naming conventions.\n"
                "- 'bluemoonbird' and 'bluemoon' are also treated as a possible entity cluster, but 'moonbird' is the main entity.\n"
                "- Therefore, the result should keep 'moonbird,bluemoonbird' as a single entity and remove 'bluemoon,bluesun'.\n"
                "- However, if 'bluemoon' and 'bluesun' also form a valid entity, they should remain as a cluster.\n"
                "- The key is to identify the most likely, dominant entity based on shared naming patterns, and merge accordingly.\n\n\n"
"""
            Example 1:
            moonbird-china, moonbirds_usa  
            → These are treated as the SAME entity because they share a clear brand/entity signal (“moonbird”) with geographic modifiers, which is a common naming pattern controlled by the same project/owner.

            Example 2:
            bayc359, bayc789  
            → These are treated as the SAME entity because they follow a systematic pattern: a shared prefix (“bayc”) plus numeric variation, which strongly suggests a single service provider, project, or owner managing multiple names.

            What should NOT be merged:

            Example 3 (WRONG merge):
            123455, 1235125  
            → These should NOT be merged because they are purely numeric strings with no identifiable entity signal (no name, brand, person, project, or semantic link).

            Example 4 (WRONG merge):
            ens, ens-web3, ens$$$  
            → These should NOT be merged because although they share “ens”, this term is too generic/broad and does not clearly point to a specific controlled entity. Generic ecosystem terms alone are insufficient for merging.
            """
        )
...
def forward_diff_prompt() -> str:
    ...
    """
    return (
        "You are an expert in ENS name-based identity analysis.\n"
        "You will be given two ENS ENTITY LINE lists (A and B). Treat them as ONE combined set.\n"
        "Each entity line is a candidate cluster (comma-separated ENS names).\n\n"
        "PRIMARY GOAL:\n"
...
def review_diff_prompt() -> str:
    ...
    """
    return (
        "You are a reviewer for ENS entity clustering based on name strings.\n"
        "You will be given current ENTITY LINES (each line is an entity; comma-separated ENS names).\n"
        "Your job is to correct mistakes based on whether names are LIKELY or POSSIBLY controlled/owned by the SAME on-chain entity.\n\n"
        "Policy:\n"
        ...
        "STRICT output format:\n"
        "- Each output line is ONE entity\n"
        "- ENS separated by commas\n"
        "- Output ONLY the lines, no explanations\n"
         "****MOST IMPORTANT:\n****"
        "During the review process, you need to evaluate the entities on a line. There may be a wrong case like:\n"
        "    \"moonbird,bluemoonbird,bluemoon,bluesun\"\n"
        "    It would be: \"moonbird,bluemoonbird\"\n\"bluemoon,bluesun\"\n"
        "In this case, consider the following logic:\n"
        "- 'moonbird' and 'bluemoonbird' are likely the SAME entity due to shared brand/entity signals and common naming conventions.\n"
...
def forward_cluster_one_bulk(ens_lines: List[str], api_key: str, system_prompt: str) -> List[str]:
    """
    Single-bulk clustering.
    NOTE: LLM output is allowed to be empty.
      - Empty output => treat as "no change" => keep all names as singletons (one per line).
    """
    user_prompt = (
        "Below is an ENS list. Treat them as ONE combined set.\n"
...
def forward_bulk_merge(ens_list: List[str], api_key: str, system_prompt: str, tokenizer) -> List[str]:
    ...
    while len(bulks) > 1:
        ...
        for j in range(1, len(bulks)):
            ...
            user_prompt = (
                "Below are two ENS ENTITY LINE lists. Treat them as ONE combined clustering.\n"
                "Cluster by whether they are LIKELY or POSSIBLY controlled/owned by the SAME on-chain entity.\n"
                "IMPORTANT (diff-only):\n"
                "- Output ONLY the entity lines that are DIFFERENT from the input grouping.\n"
                "- If nothing changes, output NOTHING.\n\n\n"
...
def review_bulk_merge_apply_deltas(entity_lines: List[str], api_key: str, tokenizer) -> List[str]:
    """
    Review pipeline (delta-accumulate, apply-once):
    - Partition entity_lines into bulks (fixed size)
    - For each pair (i, j) with i < j:
        * Ask LLM for diff-only changes between bulk i and bulk j
        * Collect returned delta clusters (do NOT update any base to avoid growing input)
    - After all pairwise comparisons, apply ALL deltas ONCE to the original entity_lines
    """
    sys_prompt = review_diff_prompt()
...
def main():
    ...
    directory_path = "/public/home/blockchain_2/slave3/deanonymization/EntityRecognition/ENS/dataset/k-means_dataset"
