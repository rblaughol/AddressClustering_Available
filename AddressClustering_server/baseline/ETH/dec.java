...
public class dec {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < addresses.size(); i++) {
            ...
            // Process initial address patterns
            matchedGroups.clear();

            List<Set<Vertex>> initGroups =
                    sg1_processor.processInitAddress(
                            ClusterAdd, g, "None", "None", strict);

            allMatchedGroups.addAll(initGroups);

            List<Set<Vertex>> midGroups =
                    sg2_processor.processMidAddress(
                            ClusterAdd, g, "None", "None", strict);

            allMatchedGroups.addAll(midGroups);

            // Process DV-based address patterns
            List<Object> DVs = g.V()
            ...
            List<Object> uniqueDVs =
                    DVs.stream().distinct().collect(Collectors.toList());
            ...
            for (Object dvObj : uniqueDVs) {
                ...
                List<Set<Vertex>> dvGroups =
                        sg1_processor.processInitAddress(
                                ClusterAdd, g, "TT", DV, strict);

                allMatchedGroups.addAll(dvGroups);

                List<Set<Vertex>> dvGroups_2 =
                        sg2_processor.processMidAddress(
                                ClusterAdd, g, "TT", DV, strict);
